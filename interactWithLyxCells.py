...
import sys, os, time, datetime, getpass
import random, string # just for generating random filenames
...
class InteractWithLyxCells(object):
   ...
   def __init__(self, clientName):
      ...
      while True:
         if self.getServerEvent() == None: break
   ...
   def getAllCellTextFromLyxFile(self, filename):
      ...
      setNextCellCookieLineBefore = -1
      lyxFile = open(filename, "r")
      foundEndDocument = False
      while True:
         ...
         # or a cookie at the start of a cell line or anywhere in an ordinary line
            ...
            if line.find(self.magicCookie) != -1: # found cookie anywhere on line
               if len(cellList) > 0: cellList[-1].cookieLineAfter = lineNumber
               setNextCellCookieLineBefore = lineNumber
      return cellList
